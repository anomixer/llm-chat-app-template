...
- 🌙 **다크 모드**：라이트/다크 테마 토글
- 📝 **Markdown 지원**：완전한 markdown 렌더링
- 🏷️ **메시지 라벨**：명확한 사용자/AI 메시지 식별
- 🚨 **Toast 알림**：비침입적 오류 메시지
- ⏹️ **스트림 취소**：정지 버튼으로 AI 생성 중단 가능
- 📊 **실시간 지표**：라이브 token 카운트 및 생성 속도 (tokens/s)
- ⚡ **스마트 버퍼링**：최적화된 UI 업데이트 (50ms 배치 처리)로 60+ FPS 부드러운 성능 구현
- 📄 **다국어 문서**：5개 언어 README 파일
...
| 지표 | 값 |
|--------|-------|
| **UI 업데이트 빈도** | 50ms마다 배치 처리 (vs. token마다) |
| **DOM 작업** | 99%+ 감소 |
| **프레임 레이트** | 안정적인 60+ FPS |
...
```typescript
const MODEL_ID = "@hf/google/gemma-7b-it"; // 여기를 수정
```
...
시스템 프롬프트는 사용자 언어에 따라 자동으로 로컬라이즈됩니다。`public/chat.js`의 `SYSTEM_PROMPT` 객체를 업데이트하세요:
...
`public/chat.js`의 버퍼링 간격을 수정:

```javascript
const updateInterval = 50; // 밀리초 (기본값: 50ms)
```

낮은 값 = 더 빈번한 업데이트 (높은 CPU 사용률)  
높은 값 = 덜 빈번한 업데이트 (부드럽지만 덜 실시간)
...
사용자는 AI 생성 중에 정지 버튼 (⏹️)을 클릭하여 스트림을 취소할 수 있습니다。이는 `AbortController`로 구현되었습니다:
...
애플리케이션은 다음을 표시합니다:
- **Token 카운트**: 생성된 token 수
- **생성 속도**: 실시간 초당 token 수
